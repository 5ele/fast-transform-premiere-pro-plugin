((root) => {
	...
	if (usePoly || !self.MessageChannel) {
		var isWindowToWindowMessage = (currentTarget) =>
		...
		MessagePort.prototype = {
			start: function () {
				...
				setTimeout(() => {
					this.log(
						"draining " + this._messageQueue.length + " queued messages",
					);
					while (
						this._messageQueueEnabled &&
						(event = this._messageQueue.shift())
					) {
			...
			_getPort: (portClone, messageEvent, copyEvents) => {
				var loadPort = (uuid) => {
    ...
    */
		MessageChannel.encodeEvent = (data, ports, messageChannel) => {
		...
		MessageChannel._messageEvent = (data, ports, messageChannel) => ({
			data: data,
			ports: ports,
			messageChannel: messageChannel,
		});

		MessageChannel._strippedPort = (port) => {
			if (!port) {
				return;
			}
    ...
    */
		MessageChannel.decodeEvent = (messageEvent, copyEvents) => {
			var fakeEvent = {
					data: null,
					ports: [],
				},
    ...
    */
		var decodeEvent = (event, copyEvents) => {
			var messageEvent;
		...
		var _addMessagePortEventHandler = (target) => {
		...
		var _overrideMessageEventListener = (target) => {
			...
			target[addEventListenerName] = function () {
				...
				if (args[0] === messageEventType) {
					messageHandlerWrapper = (event) => {
						var messageEvent = decodeEvent(event);

						if (!messageEvent.messageChannel) {
							originalHandler.call(this, messageEvent);
						}
					};
    ...
    */
		if (Window) {
			Window.postMessage = (otherWindow, message, targetOrigin, ports) => {
		...
		if (self.Worker) {
			self.Worker.postMessage = (worker, message, transferList) => {
		...
		if (Window) {
			Window.postMessage = (source, message, targetOrigin, ports) => {
				// Internet Explorer requires the `ports` parameter
				ports = ports || [];
